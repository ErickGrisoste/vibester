...
- rodar, a cada hora (`0 * * * *`, cron via `node-cron`, timezone `env.timezone`), um job que:
  ...
  2. para cada estabelecimento com `googlePlaceId`, consulta a SerpAPI (engine `google_maps`) para obter popularidade ao vivo (`live_busyness_score`) e o histórico horário do dia (`popular_times`), com a resposta validada por Zod (ver `SerpApiService`, seção Segurança #10);
  3. calcula o movimento (`computeMovement` em `movement-engine.ts`) a partir do score ao vivo — ou, se ausente, da **mediana** das amostras históricas do mesmo dia da semana/hora dos últimos 7 dias (tabela `popular_times_daily`, mínimo de 3 amostras; menos que isso vira `UNAVAILABLE`). O score ao vivo passa por suavização (EMA, α=0.6, contra o score suavizado do ciclo anterior) e a troca de faixa (`VERY_LOW`…`VERY_HIGH`) passa por histerese (uma margem de 3 pontos perto do limiar não troca a faixa se a mudança for de banda vizinha — mudanças maiores aplicam na hora). O resultado inclui um `confidence` (0–1: 1.0 para score ao vivo, 0.5×proporção de amostras/7 para estimado) — ver `src/services/movement-engine.ts` para a implementação completa e `src/services/__tests__/movement-engine.test.ts` para os casos cobertos;
...
9. **Evento Kafka publicado (`establishment.movement.updated`) não deve carregar dado sensível** — hoje carrega `establishmentId`, `level`, `source`, `confidence` e, opcionalmente, `category`; não adicione campos internos (ex.: `googlePlaceId` cru, dados de billing da SerpAPI) ao payload sem necessidade real do lado consumidor.
