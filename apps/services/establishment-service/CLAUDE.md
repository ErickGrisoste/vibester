...
- `k8s/`: `deployment.yaml` (`readinessProbe` em `/health/ready` checando DB + Redis, `livenessProbe` em `/health`), `hpa.yaml` (hoje `minReplicas: 2` e `maxReplicas: 2` — ou seja, **autoscaling efetivamente fixo em 2 réplicas** no momento; não assuma que o serviço escala horizontalmente sob carga até que esses valores sejam ajustados), `pdb.yaml`, `service.yaml`, e **`networkpolicy.yaml`** (único entre os serviços documentados até agora — restringe ingress/egress a nível de pod).
