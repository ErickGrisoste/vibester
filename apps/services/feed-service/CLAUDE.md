...
src/
  ...
  kafka/         client.ts (singleton), consumer.ts (assinatura de tópicos + dispatch para FeedFanoutService/FollowService/
                 EventAttendanceService, injetados via construtor), producer.ts (não usado)
  ...
  server.ts      bootstrap Fastify, CORS aberto, JWT, swagger, registerErrorHandler, inicia o KafkaConsumer antes do listen,
...
3. **Fan-out on write sem paginação nem limite de concorrência é o maior risco de escala atual**: `UserFollowerRepository.findFollowersByUser`/`EstablishmentFollowersRepository.findFollowersByEstablishment` fazem `SELECT follower_id FROM ... WHERE user_id/establishment_id = ?` **sem `LIMIT`**, e `distributePostToFollowers`/`distributeEventToFollowers` (`src/services/feed-fanout.service.ts`) disparam um fan-out com uma escrita Cassandra por seguidor. Para uma conta/estabelecimento com centenas de milhares de seguidores, isso vira uma única leva de centenas de milhares de writes simultâneos por post — não escala para o volume que o `CLAUDE.md` raiz assume como premissa. Isso ainda **não foi corrigido** (é a próxima fase do plano de refatoração) — se for expandir esse fluxo antes disso, pagine a leitura de seguidores e distribua em lotes com concorrência limitada, em vez de aumentar o alcance do fan-out atual.
